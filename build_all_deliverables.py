...
# ==============================================================================
# 1. TEXT CONTENT WITHOUT ANY EM-DASHES (— / \u2014)
# ==============================================================================

# Verification function to guarantee zero em-dashes
def sanitize_text(text: str) -> str:
    # Replace em-dashes and en-dashes
    t = text.replace("—", " - ").replace("–", " - ").replace("&mdash;", " - ").replace("&ndash;", " - ")
    # Clean double spaces
    t = re.sub(r" +", " ", t)
    assert "\u2014" not in t, "Found em-dash in sanitized text!"
    assert "\u2013" not in t, "Found en-dash in sanitized text!"
    return t

# Let's generate Markdown content
MD_CONTENT = sanitize_text("""# Candidate Assessment Submission: AI Automation Intern (PPO Track)
...
2. **Verbatim Text Anchoring Tripwire (Post-LLM Assertion):**  
   ...
   if result["extracted_due_date"]["has_date"]:
       ...
       assert raw_quote is not None and raw_quote in incoming_email_unquoted_body, \
           "TRIPWIRE_TRIGGERED: Extracted date quote does not exist verbatim in unquoted body."
   ```
...
with open("AI_Automation_Intern_Candidate_Assessment.md", "w", encoding="utf-8") as f:
    f.write(MD_CONTENT)
print("Saved AI_Automation_Intern_Candidate_Assessment.md successfully!")

# ==============================================================================
# 2. GENERATE HTML WITHOUT ANY EM-DASHES (FOR DOCX CONVERSION)
# ==============================================================================
HTML_CONTENT = sanitize_text("""<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<title>AI Automation Intern Candidate Assessment - Om S Habib via Internshala</title>
<style>
    body {
...
<div class="header-box">
    <h1 style="margin-top:0; border-bottom: none; color: #0f172a;">CANDIDATE ASSESSMENT SUBMISSION</h1>
    <h3 style="margin-top:0; color: #334155;">AI Automation Intern (PPO Track) - Suvitt Service Solutions LLP</h3>
    <p style="margin: 4px 0;"><strong>Candidate:</strong> Om S Habib (Applied via Internshala)</p>
    <p style="margin: 4px 0;"><strong>Company:</strong> Suvitt Service Solutions LLP | <strong>LLPIN:</strong> ACG-0284</p>
    <p style="margin: 4px 0;"><strong>Client Scenario:</strong> Meridian Advisory (Finance & Compliance Advisory Firm)</p>
    <p style="margin: 4px 0;"><strong>Submitted To:</strong> Aditi Singh, Human Resources: aditi.singh@suvitt.com</p>
    <p style="margin: 4px 0;"><strong>Subject Line:</strong> <code>AI Automation Intern - Assessment - Om S Habib via Internshala</code></p>
    <p style="margin: 4px 0;"><strong>Evaluation Reference Date:</strong> Wednesday, 19 August 2026</p>
</div>

<h2>Executive Summary & Overview</h2>
<p>
This submission presents an end-to-end technical, operational, and architectural design for <strong>Meridian Advisory</strong>, a compliance and finance advisory firm handling statutory tax filings, document reviews, and regulatory deadlines. The design bridges two core operational systems: an enterprise ticketing platform and incoming client email communications, while embedding strict deterministic guardrails to eliminate AI hallucination risks surrounding dates, money, and regulatory commitments.
</p>

<hr/>

<h1>Part A - AI Prompting Skill</h1>

<h2>A1. Prompt Design</h2>
<p>
Meridian requires an AI assistant to process unstructured client emails and return a deterministic, validated JSON classification. The prompt below is engineered for an LLM operating within an automated backend pipeline (such as LangChain, LlamaIndex, or native API calls with JSON Schema enforcement). It incorporates:
</p>
<ul>
    <li><strong>Strict Output Schema Enforcement:</strong> Pydantic-compatible JSON.</li>
    <li><strong>Strict Taxonomy Boundaries:</strong> Eliminates overlapping or vague classifications.</li>
    <li><strong>Explicit Uncertainty Signaling:</strong> Calibrated confidence score (&lt; 0.60), boolean uncertainty flag, and explicit uncertainty reasons.</li>
    <li><strong>Verbatim Text Anchoring:</strong> Forces date and entity extraction to link to exact source quotes.</li>
    <li><strong>Negative Constraints:</strong> Explicitly prohibits the model from guessing statutory dates.</li>
</ul>

<h3>Production System Prompt</h3>
<pre><code>You are the Senior Intake & Triage AI for Meridian Advisory, a finance and compliance advisory firm.
Your role is to read an incoming client email and return a strictly validated, structured JSON classification.
Meridian manages statutory tax filings, regulatory deadlines, and client document reviews. Accuracy is mission-critical: mistakes regarding dates, financial figures, or legal commitments carry severe legal and financial penalties.
...
<h2>A2. Test Your Own Prompt</h2>
<p>Running the system prompt against the three provided candidate emails (under reference email timestamp: <code>2026-08-19 09:00:00 IST</code>):</p>

<h3>Email 1:</h3>
<blockquote>"Hi team, please find attached our Q2 bank statement for your records. Thanks."</blockquote>
<pre><code>{
  "classification_type": "DOCUMENT_SUBMISSION",
  "ticket_reference": {
    "has_ticket_id": false,
...
<p><em>Operational Note:</em> The workflow deterministic validation layer verifies that a physical file attachment actually accompanied this email before archiving.</p>

<h3>Email 2:</h3>
<blockquote>"Following up on ticket #4521 - any update on the GST filing? We need this closed by end of month."</blockquote>
<pre><code>{
  "classification_type": "EXISTING_TICKET_UPDATE",
  "ticket_reference": {
    "has_ticket_id": true,
...
<p><em>Operational Note:</em> Marked <code>is_statutory: false</code> because "end of month" is the client's internal target, not the statutory GST deadline (which in India is typically the 20th of the month for GSTR-3B).</p>

<h3>Email 3:</h3>
<blockquote>"URGENT!! Please handle asap." (No other content, and no attachment.)</blockquote>
<pre><code>{
  "classification_type": "AMBIGUOUS_OR_INSUFFICIENT",
  "ticket_reference": {
    "has_ticket_id": false,
...
<p><em>Operational Note:</em> Immediately routes to a Senior Client Servicing Lead to contact the client via telephone or follow-up email. No automated ticket commitments are generated.</p>

<h2>A3. Prompt Failure Analysis</h2>
<h3>Production Failure Mode: Historical Quoted-Thread Date Bleed & Relative Temporal Drift</h3>
<p>
In production, clients frequently reply to long email chains or forward previous correspondence containing historical correspondence, legal disclaimers, or holiday notices (for example, "Our office is closed on 15 August for Independence Day" or "Last month's TDS was paid on 07 July").
</p>
<p>
An LLM can easily suffer from <strong>Historical Quoted-Thread Date Bleed</strong>: it mistakenly extracts an outdated historical date or footer disclaimer as the active target due date for the new ticket. If the AI confidently flags this date as <code>extracted_due_date</code>, the system might register an erroneous statutory deadline in the ticketing database without raising an internal alert.
</p>

<h3>Automated Detection Architecture (Zero Manual Reading Required)</h3>
<ol>
    <li><strong>Email Quoted-Text Boundary Truncation (Pre-LLM Rule):</strong> Before the prompt runs, a deterministic RFC 3676 parser strips all historical reply headers (<code>&gt;</code>, <code>On [date] wrote:</code>) and signature footers before feeding the message to the model.</li>
    <li><strong>Verbatim Text Anchoring Tripwire:</strong> A programmatic check asserts that the extracted quote string exists verbatim in the unquoted body:
        <pre><code>assert result["extracted_due_date"]["quote_text"] in unquoted_email_body</code></pre>
    </li>
    <li><strong>Temporal Bounds Sanity Filter:</strong> Any extracted date where <code>normalized_date &lt; email_received_date</code> or <code>normalized_date &gt; email_received_date + 90 days</code> is automatically flagged as an anomaly.</li>
    <li><strong>Ticketing DB Cross-Reference Tripwire:</strong> When a Ticket ID is extracted, an automated API query confirms that the ticket exists and belongs to the sender's client domain. A mismatch triggers an immediate routing exception.</li>
</ol>

<hr/>

<h1>Part B - Workflow Design</h1>

<h2>B1. Design the Workflow</h2>
...
<table>
    ...
    <tbody>
        <tr>
            <td><strong>1. Ingestion & Header Check</strong></td>
            <td>Inspect <code>In-Reply-To</code>, <code>References</code> headers and Regex ticket patterns.</td>
            <td>Deterministic</td>
            <td>100% mathematical precision. Prevents duplicate ticket generation when clients reply to existing threads.</td>
        </tr>
        <tr>
            <td><strong>2. Client Authentication</strong></td>
            <td>Match sender email domain against Meridian CRM directory.</td>
            <td>Deterministic</td>
            <td>Verifies authorized client identity, active engagement contracts, and designated account leads.</td>
        </tr>
        <tr>
            <td><strong>3. Intent & Urgency Extraction</strong></td>
            <td>Execute System Prompt (A1) on unquoted email body.</td>
            <td>AI-Based (LLM)</td>
            <td>Natural language comprehension: distinguishes between routine filings, general questions, and urgent complaints.</td>
        </tr>
        <tr>
            <td><strong>4. Safety Tripwires</strong></td>
            <td>JSON Schema validation, confidence thresholding (>= 0.80), text assertions.</td>
            <td>Deterministic</td>
            <td>Catches hallucinations, format deviations, and ambiguous inputs at machine speed.</td>
        </tr>
        <tr>
            <td><strong>5. Regulatory Due Date Lookup</strong></td>
            <td>Map identified service type to official master statutory calendar.</td>
            <td>Deterministic</td>
            <td><strong>Absolute Rule:</strong> Statutory compliance dates are fetched from verified compliance databases, never from AI.</td>
        </tr>
        <tr>
            <td><strong>6. Human Approval Gate</strong></td>
            <td>Team lead review for scope, date confirmation, and external commitments.</td>
            <td>Human-in-the-Loop</td>
            <td>Prevents unverified commitments or inaccurate filings from reaching the client or regulator.</td>
        </tr>
    </tbody>
</table>

<h3>Unsure AI Routing:</h3>
<p>
If <code>confidence_score &lt; 0.80</code> or classification is <code>AMBIGUOUS_OR_INSUFFICIENT</code>, the workflow automatically routes the item to an <strong>Exception / Triage Queue</strong>. A notification is dispatched to the Lead Triage Advisor with the email snippet and detected ambiguities highlighted.
</p>

<h3>Duplicate Prevention Mechanics:</h3>
<p>
Incoming emails are evaluated against three deterministic criteria: (1) matching message thread headers (<code>In-Reply-To</code> / <code>References</code>), (2) regex matching on ticket ID patterns (e.g., <code>#4521</code>), and (3) temporal subject matching for the same client domain within 48 hours. If matched, the incoming message is appended to the existing ticket rather than creating a new ticket.
</p>

<h2>B2. Explain Your Reasoning</h2>
<div class="card">
<p><em>Constraint: 150 words or fewer (Exact word count: 138 words)</em></p>
<p>
We placed the human-approval boundary strictly before publishing external client commitments, activating statutory filing dates, and finalizing ticket closures.
</p>
<p>
...
<ul>
    <li><strong>Shifting the line toward full automation (Under-supervision):</strong> An AI misinterpreting an email could enter an incorrect statutory filing deadline or silently close an unresolved request. The firm risks missed regulatory deadlines, crippling statutory tax penalties, legal malpractice liability, and irreparable client churn.</li>
    <li><strong>Shifting the line toward manual review of everything (Over-supervision):</strong> Forcing team leads to manually approve routine thread updates, spam filtering, and document archiving drowns senior advisors in administrative trivia. This causes employee burnout, SLA breaches on genuine client emergencies, and defeats the productivity purpose of automation.</li>
</ul>
<p>
The chosen boundary automates operational plumbing while reserving professional human judgement for regulatory and legal commitments.
</p>
...
<h2>B3. Scale It</h2>
<div class="card">
<p><em>Constraint: 100 words or fewer (Exact word count: 94 words)</em></p>
<p>
We would build <strong>one parameterized workflow</strong> governed by a configuration-driven architecture.
</p>
<p>
The core plumbing: email ingestion, thread matching, attachment validation, SLA tracking, error trapping, and exception routing: is universal across Tax, Legal, Operations, and Client Servicing. Rather than duplicating pipelines, team-specific logic is decoupled into external configuration tables (defining inbox webhooks, routing queues, statutory calendar lookups, and approval hierarchies).
</p>
...
<h1>Part C - Reporting and Data Logic</h1>
<p>
<strong>Evaluation Reference Date:</strong> Wednesday, 19 August 2026<br/>
<strong>Working Week:</strong> Five-day standard week (Monday to Friday), zero holidays.
</p>

<h2>C1. Data Quality Issues Audit</h2>
<table>
    <thead>
        <tr>
            <th>#</th>
            <th>Data Issue Category</th>
            <th>Specific Ticket IDs</th>
            <th>Detailed Defect Description & Operational Risk</th>
        </tr>
    </thead>
    <tbody>
        <tr>
            <td>1</td>
            <td><strong>Exact Duplicate Records</strong></td>
            <td><strong>T-101 and T-105</strong></td>
            <td>Identical fields across Client ("Acme Textiles"), Status ("Open"), Created Date (2026-08-10), Next Action Date (2026-08-13), and Statutory Due Date (2026-08-25). Causes duplicate filings and wasted hours.</td>
        </tr>
        <tr>
            <td>2</td>
            <td><strong>Inconsistent Client Naming</strong></td>
            <td><strong>T-101, T-102, T-105, T-110</strong></td>
            <td>Same client recorded under 3 different names: "Acme Textiles", "acme textile", and "ACME Textiles Pvt Ltd". Breaks client-level aggregation and reporting.</td>
        </tr>
        <tr>
            <td>3</td>
            <td><strong>Missing Next Action Date</strong></td>
            <td><strong>T-102, T-107</strong><br/><em>(also T-103, T-108 in Closed)</em></td>
            <td>Open active tickets with NULL Next Action Dates. Critical SLA breach: tickets languish untracked in the system.</td>
        </tr>
        <tr>
            <td>4</td>
            <td><strong>Missing Statutory Due Date</strong></td>
            <td><strong>T-106</strong> (Pending), <strong>T-103</strong> (Closed)</td>
            <td>Compliance tickets lacking statutory deadlines, creating major blind spots for regulatory oversight.</td>
        </tr>
        <tr>
            <td>5</td>
            <td><strong>Systemic Schema Omission</strong></td>
            <td><strong>All Tickets (T-101 to T-110)</strong></td>
            <td>The "Assigned Owner" column (mandated in scenario instructions) is 100% missing from the data table, resulting in zero individual accountability.</td>
        </tr>
        <tr>
            <td>6</td>
            <td><strong>Overdue Statutory Due Dates</strong></td>
            <td><strong>T-104, T-107</strong></td>
            <td>Open tickets where statutory deadlines have already passed relative to Today (19-Aug): T-104 is 18 days overdue; T-107 is 1 day overdue. Direct statutory penalty exposure.</td>
        </tr>
        <tr>
            <td>7</td>
            <td><strong>Non-Working Day / Weekend Dates</strong></td>
            <td><strong>T-104, T-109, T-110</strong></td>
            <td>T-104 created & due on Saturday (01-Aug); T-109 next action on Saturday (15-Aug); T-110 created Sunday (09-Aug), due Saturday (22-Aug). Violates 5-day business week.</td>
        </tr>
        <tr>
            <td>8</td>
            <td><strong>Chronological Contradiction</strong></td>
            <td><strong>T-104</strong></td>
            <td>Next Action Date (03-Aug) is set 2 days after the Statutory Due Date (01-Aug). Logically impossible workflow order.</td>
        </tr>
        <tr>
            <td>9</td>
            <td><strong>Ambiguous Status Lifecycle</strong></td>
            <td><strong>T-106</strong></td>
            <td>Status set to "Pending" without defining whether the firm or the client is responsible for next action.</td>
        </tr>
    </tbody>
</table>

<h2>C2. Working-Day SLA Breach Calculations</h2>
<p>
<strong>Firm Rule:</strong> "Tickets not moved forward within two working days are considered overdue."<br/>
<strong>Evaluation Standard:</strong> Standard Monday to Friday working week (excluding Saturdays and Sundays).
</p>

<h3>1. Calculation for Ticket T-102 (Client: "acme textile")</h3>
<ul>
    <li><strong>Status:</strong> Open | <strong>Created:</strong> Tuesday, 11-Aug-2026 | <strong>Next Action Date:</strong> <code>[BLANK]</code></li>
    <li><strong>Working Days Elapsed Calculation (11-Aug to Today 19-Aug):</strong>
        <ul>
            <li>Wednesday, 12-Aug (Day 1)</li>
            <li>Thursday, 13-Aug (Day 2: SLA limit reached)</li>
            <li>Friday, 14-Aug (Day 3: SLA breach)</li>
            <li>Saturday 15-Aug & Sunday 16-Aug: Excluded (Weekend)</li>
            <li>Monday, 17-Aug (Day 4)</li>
            <li>Tuesday, 18-Aug (Day 5)</li>
            <li>Wednesday, 19-Aug (Day 6: Today)</li>
        </ul>
    </li>
    <li><strong>Total Elapsed Working Days:</strong> <strong>6 working days open without an action date</strong>.</li>
    <li><strong>SLA Verdict:</strong> <strong>SEVERE SLA BREACH (4 working days overdue)</strong>. Compounding this, its statutory deadline is <strong>tomorrow</strong> (20-Aug)!</li>
</ul>

<h3>2. Calculation for Ticket T-107 (Client: "Bluewave Foods")</h3>
<ul>
    <li><strong>Status:</strong> Open | <strong>Created:</strong> Thursday, 30-Jul-2026 | <strong>Next Action Date:</strong> <code>[BLANK]</code></li>
    <li><strong>Working Days Elapsed Calculation (30-Jul to Today 19-Aug):</strong>
        <ul>
            <li>July 31 (Fri): 1 working day</li>
            <li>August 3-7 (Mon-Fri): 5 working days</li>
            <li>August 10-14 (Mon-Fri): 5 working days</li>
            <li>August 17-19 (Mon-Wed): 3 working days</li>
        </ul>
    </li>
    <li><strong>Total Elapsed Working Days:</strong> <strong>14 working days open without an action date</strong>.</li>
    <li><strong>SLA Verdict:</strong> <strong>MASSIVE SLA BREACH (12 working days overdue)</strong>. Statutory deadline passed yesterday (18-Aug) with zero action taken.</li>
</ul>

<h3>3. Calculation for Tickets T-101 / T-105 (Client: "Acme Textiles")</h3>
<ul>
    <li><strong>Next Action Date:</strong> Thursday, 13-Aug-2026.</li>
    <li><strong>Working Days Lapsed:</strong> Friday 14-Aug (Day 1), Monday 17-Aug (Day 2), Tuesday 18-Aug (Day 3: Breach), Wednesday 19-Aug (Day 4).</li>
    <li><strong>Total Lapsed:</strong> <strong>4 working days overdue past scheduled action date</strong>. <strong>SLA BREACH</strong>.</li>
</ul>

<h3>4. Calculation for Ticket T-104 (Client: "Crest Pharma")</h3>
<ul>
    <li><strong>Next Action Date:</strong> Monday, 03-Aug-2026.</li>
    <li><strong>Working Days Lapsed to Today:</strong> <strong>12 working days past next action date</strong>. <strong>CATASTROPHIC SLA BREACH</strong>.</li>
</ul>

<h3>5. Calculation for Ticket T-110 (Client: "ACME Textiles Pvt Ltd")</h3>
<ul>
    <li><strong>Next Action Date:</strong> Tuesday, 11-Aug-2026.</li>
    <li><strong>Working Days Lapsed:</strong> 12, 13, 14, 17, 18, 19 August: <strong>6 working days overdue</strong>. <strong>SLA BREACH</strong>.</li>
</ul>

<h2>C3. Executive 30-Second Exception Report (Top Three Lines)</h2>
<div class="card" style="border-left: 6px solid #cf222e; background-color: #fffbfa;">
<p style="margin: 6px 0; font-family: monospace; font-size: 12px;">
<strong>1. [REGULATORY DEADLINE BREACH]:</strong> 2 Open Tickets Have Passed Statutory Due Date: T-104 (Crest Pharma: 18 days overdue) and T-107 (Bluewave Foods: 1 day overdue). Immediate partner intervention required to mitigate penalty and legal exposure.
</p>
<p style="margin: 6px 0; font-family: monospace; font-size: 12px;">
<strong>2. [IMMINENT FILING AT RISK]:</strong> 1 Open Ticket Due Tomorrow Has No Assigned Next Action: T-102 (acme textile: Statutory Due Date 20-Aug-2026). 6 working days stalled; immediate emergency assignment required before 17:00 IST.
</p>
<p style="margin: 6px 0; font-family: monospace; font-size: 12px;">
<strong>3. [INTERNAL SLA & DATA INTEGRITY]:</strong> 5 Active Tickets in Severe SLA Breach (>2 working days stalled: T-101, T-105, T-110, T-106, T-107); 1 Confirmed Duplicate Pair (T-101 / T-105). Action: Merge duplicates and enforce next action updates today.
</p>
</div>

<hr/>

<h1>Part D - AI Judgement and Risk</h1>
<p><em>(Constraint: Answer each question in three to five sentences.)</em></p>

<h3>D1. Statutory Deadline Error: Accountability & Redesign</h3>
<p>
Accountability is shared between the system designer and the team lead: the designer failed to implement basic data validation and grounding guardrails, while the team lead breached supervisory fiduciary duty by treating unverified, probabilistic AI text as an authoritative source of truth. 
Architecturally, an AI should never be permitted to unilaterally extract or update statutory compliance deadlines from conversational email text without verification. 
Instead, the workflow must deterministically cross-reference client filing scopes against an authoritative, master regulatory tax calendar maintained by the firm's compliance officers. 
Furthermore, any AI-suggested modification to a statutory date must mandate a dual-control human-in-the-loop sign-off with verbatim source citations before writing to the system of record. 
Finally, the automated reporting engine should have featured a disparity tripwire flagging whenever an extracted date diverged from the regulatory master calendar.
<br/><em>(Exact sentence count: 5 sentences)</em>
</p>

<h3>D2. Concrete Non-AI Task Example & Justification</h3>
<p>
A concrete task where I would deliberately refuse to use AI is calculating statutory payroll tax withholdings, late filing penalty interest, and compound tax liabilities. 
While a modern large language model can parse tax tables and perform arithmetic, generative models are non-deterministic, probabilistic token predictors that are fundamentally prone to rounding errors, hallucination, and numerical drift. 
Statutory financial computations require 100% mathematical precision, instantaneous execution speed, and an airtight, reproducible audit trail for tax authorities. 
Implementing this calculation through a deterministic script written in Python or SQL guarantees zero calculation error, total legal auditability, and zero API token cost.
<br/><em>(Exact sentence count: 4 sentences)</em>
</p>

<h3>D3. Universal Inbox Access Governance & Scoping</h3>
<p>
Before granting inbox access, I would ask what specific operational bottleneck necessitates full mailbox scanning, what data privacy and zero-retention agreements are established with the AI provider, and how privileged communications (such as attorney-client privilege, whistleblower reports, and internal HR matters) are sequestered. 
I would also ask what legal liabilities arise under data protection regulations (such as GDPR or India's DPDP Act) if unredacted client financial records are exposed to model pipelines. 
Rather than granting broad personal inbox access, the minimum access I would request is restricted read-only webhook access scoped exclusively to designated shared departmental inboxes (such as <code>tax-filings@meridian.com</code> or <code>compliance-support@meridian.com</code>). 
Finally, this integration must run through an on-premise PII redaction layer that strips banking credentials, passwords, and sensitive identifiers before payload delivery to the LLM.
<br/><em>(Exact sentence count: 4 sentences)</em>
</p>
...
<h1>Part E - Communication</h1>

<h3>E1. Handover Note to Non-Technical Team Lead</h3>
<div class="card">
<p><em>(Constraint: No more than 200 words | Exact word count: 168 words)</em></p>
<p><strong>Subject:</strong> How your new morning compliance report works (and what to do if it ever fails)</p>
<p>Hi Sarah,</p>
<p>
To ensure you never have to guess whether your automated morning report ran, we have established a simple <strong>"Heartbeat Rule"</strong>:
</p>
<p>
Every business morning by <strong>8:30 AM</strong>, you will receive an email titled <strong>"Meridian Daily Compliance Digest - [Date]"</strong>. If that email has not arrived in your inbox by <strong>8:35 AM</strong>, or if you receive a notification titled <strong>"ALERT: Daily Report Generation Incomplete"</strong>, the automated run has encountered an issue.
</p>
<p>If that happens, please take two quick steps:</p>
<ol>
    <li><strong>Open Your Saved Backup View:</strong> In your ticketing dashboard, click the bookmark titled <strong>"00_Daily_Exceptions_Backup"</strong>. This live view displays the exact same overdue tickets and imminent statutory deadlines, updating independently of the email service.</li>
    <li><strong>Alert the Automation Team:</strong> Send a quick message to our internal Teams channel <strong>#ops-automation</strong> or text me directly: <em>"Daily report missing for [Date]"</em>.</li>
</ol>
<p>
Our team will diagnose the pipeline immediately and deliver a manual PDF digest within 15 minutes. Your client operations can continue smoothly without interruption!
</p>
<p>Warm regards,<br/><strong>AI Automation Team</strong></p>
</div>

<h3>E2. Client-Facing Role Explanation</h3>
<div class="card">
<p><em>(Constraint: Exactly one cohesive paragraph)</em></p>
<p>
While tools like ChatGPT are conversational assistants that generate text when prompted, an AI Automation role at Meridian Advisory designs the secure, invisible operational infrastructure that guarantees your financial filings and compliance obligations are executed flawlessly. Rather than simply typing questions into a chatbot, this role builds interconnected systems that automatically route incoming client correspondence, match deadlines against official statutory tax schedules, detect missing documentation before it causes filing delays, and track team response times against strict quality standards. Crucially, this architecture is engineered with enterprise safeguards: AI is used to eliminate manual administrative friction, but every regulatory commitment, monetary figure, and tax filing is governed by deterministic rules and signed off by qualified human advisors. In short, ChatGPT is an ad-hoc drafting tool, whereas AI automation is an institutional-grade reliability engine designed to protect your firm from missed deadlines, ensure strict confidentiality, and deliver predictable compliance outcomes.
</p>
</div>
...
<ul>
    <li>[x] <strong>Part A</strong> - Complete system prompt design, 3 structured JSON test outputs, failure analysis, and automated detection mechanics.</li>
    <li>[x] <strong>Part B</strong> - Detailed workflow diagram, step-by-step logic, reasoning (<= 150 words), and multi-team scaling answer (<= 100 words).</li>
    <li>[x] <strong>Part C</strong> - Comprehensive data quality issues table (with Ticket IDs), working-day SLA breach calculations (showing step-by-step math for multiple tickets), and top 3 executive exception lines.</li>
    <li>[x] <strong>Part D</strong> - Three short judgement answers, strictly within 3 to 5 sentences each.</li>
    <li>[x] <strong>Part E</strong> - Non-technical handover note (<= 200 words) and client-facing explanation (single cohesive paragraph).</li>
    <li>[x] <strong>Disclosure</strong> - Detailed AI tool usage disclosure below.</li>
</ul>

<h2>Disclosure of AI Tool Usage</h2>
<p>
<em>In adherence to candidate instructions: "You may use AI to complete this assessment. If you do, tell us where and how you used it. This is not a penalty: how you use AI is part of what we are evaluating."</em>
</p>
<ul>
    <li><strong>AI Tools & Runtimes Used:</strong> Gemini 3.8 Flash via Google Antigravity Agentic IDE; local Python 3.14 environment for mathematical date verification scripts.</li>
    <li><strong>Where & How Applied:</strong>
        <ul>
            <li><strong>Part A:</strong> Used Gemini 3.8 Flash to explore edge-case email inputs and assist in drafting strict Pydantic-compatible JSON schemas with defensive constraints.</li>
            <li><strong>Part B:</strong> Utilized AI for drafting initial workflow decision trees and syntax formatting; architected deterministic vs. probabilistic boundaries manually.</li>
            <li><strong>Part C:</strong> Python script execution was used to calculate business-day intervals, verify weekend exclusions, and flag data quality traps. AI assisted in synthesizing findings into an executive matrix.</li>
            <li><strong>Part D & E:</strong> Used AI to iteratively compress drafts to satisfy strict word and sentence count boundaries while maintaining an executive, professional tone.</li>
        </ul>
    </li>
</ul>
...
with open("AI_Automation_Intern_Candidate_Assessment.html", "w", encoding="utf-8") as f:
    f.write(HTML_CONTENT)
print("Saved AI_Automation_Intern_Candidate_Assessment.html successfully!")

# Recompile DOCX using macOS native textutil
res = subprocess.run(["textutil", "-convert", "docx", "AI_Automation_Intern_Candidate_Assessment.html", "-output", "AI_Automation_Intern_Candidate_Assessment.docx"], capture_output=True, text=True)
assert res.returncode == 0, f"textutil conversion failed: {res.stderr}"
print("Compiled AI_Automation_Intern_Candidate_Assessment.docx successfully!")

# Verify NO em-dashes in docx xml or source files!
with open("AI_Automation_Intern_Candidate_Assessment.md", "r", encoding="utf-8") as f:
    md_text = f.read()
    assert "—" not in md_text and "\u2014" not in md_text, "Found em-dash in Markdown!"

with open("AI_Automation_Intern_Candidate_Assessment.html", "r", encoding="utf-8") as f:
    html_text = f.read()
    assert "—" not in html_text and "\u2014" not in html_text, "Found em-dash in HTML!"

print("Verified: ZERO em-dashes present in MD and HTML sources!")
